# Project Quartermaster — Quickstart (v0.5.1)

This guide takes you from a clean checkout to a running Telegram bot with
working general commands (`/start`, `/help`, `/me`, `/menu`, `/register`), a
seeded admin account, seeded categories/locations, and the inventory commands
(`/add_item`, `/search_item`, `/view_item`, `/update_item`, `/archive_item`).
...
Version 0.5.1 adds **self-service registration + user management**: new users
register themselves (full name → NIM) and the main admin approves them with a
role from a 🛡 Admin panel. By the end of this guide:
...
[Nest] LOG [Bootstrap] Project Quartermaster bot is running (v0.5.1)
```
...
See the roadmap in `Design.md` (Section 29). This release delivered **v0.5.1 —
Self-Service Registration & User Management** (`/register`, the main-admin 🛡
Admin panel / `/requests`, role assignment and user management), on top of the
v0.5.0 Reports release. The next milestone is **v0.6.0 — Audit Logs**
(traceable changes via an `AuditLog` model + an admin audit view).
